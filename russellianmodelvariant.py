    ...
    for Z in range(1, 121):
        ...
        if delta_phi > FRACTURE_THRESHOLD or S_H >= T2 or S_C >= T2:
            ...
            zx_fracture_nodes.append({
                ...
                "x": r_x * math.cos(theta_x), "y": r_x * math.sin(theta_x), "z": Z, 
                "Color": "#ff00ff" if delta_phi > FRACTURE_THRESHOLD else "#00ffff" # Magenta for Phase Negator, Cyan for standard instability
            })
            
    return nodes, synergy_bridges_x, synergy_bridges_y, synergy_bridges_z, zx_fracture_nodes

def render_unified_ui(nodes, syn_x, syn_y, syn_z, zx_nodes):
    fig = go.Figure()
    ...
    # Trace Index: 0:H-Path, 1:C-Path, 2:H-Nodes, 3:C-Nodes, 4:Bridges, 5:Fractures
    updatemenus = [
        dict(
            ...
            buttons=list([
                ...
                dict(label="[3] Biological (C-Wave)", method="update", 
                     args=[{"visible": [False, True, False, True, False, False]}]),
                dict(label="[4] Fracture Test (Arsenic/Negators)", method="update", 
                     args=[{"visible": [True, True, False, False, False, True]}]),
            ]),
            font=dict(color="#ffffff"), bgcolor="rgba(20,20,40,0.9)", bordercolor="#666688"
        )
    ]
...
if __name__ == "__main__":
    nodes, syn_x, syn_y, syn_z, zx_nodes = generate_unified_data()
    render_unified_ui(nodes, syn_x, syn_y, syn_z, zx_nodes)